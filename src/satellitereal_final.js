...
class real_Satellite {
	constructor(scene) {
		this.scene = scene;
        this.satelliteM = this.createSatellite();
        this.sateSpeed = this.createSpeed();
        this.line = [];
    ...
    createSatellite() {
        ...
        var OneSatellite = BABYLON.MeshBuilder.CreateBox("OneSatellite", {}, scene);
        
        OneSatellite.position.z = 36;
        OneSatellite.position.x = 0;
        
        var Meterial_sate = new BABYLON.StandardMaterial("Meterial_sate", scene);
        Meterial_sate.diffuseColor = new BABYLON.Color3(0.3, 0.95, 0.2);
        OneSatellite.material = Meterial_sate;
        
        var solar1 = BABYLON.MeshBuilder.CreatePlane("solar1", {height:0.75, width: 2.5, sideOrientation: BABYLON.Mesh.DOUBLESIDE}, scene);
        solar1.parent = OneSatellite;
        solar1.position.x = 2;
        var solar2 = BABYLON.MeshBuilder.CreatePlane("solar2", {height:0.75, width: 2.5, sideOrientation: BABYLON.Mesh.DOUBLESIDE}, scene);
        solar2.parent = OneSatellite;
        solar2.position.x = -2;
        ...
        this.nozzle1.parent = OneSatellite;
        ...
        this.nozzle2.parent = OneSatellite;
        ...
        this.nozzle3.parent = OneSatellite;
        ...
        this.nozzle4.parent = OneSatellite;
        this.nozzle4.rotation.x = Math.PI/2;
        this.nozzle4.rotation.y = Math.PI/2;
        this.nozzle4.position.x = -0.55;
        
        var msolars = new BABYLON.StandardMaterial("msolars", scene);
        //msolars.diffuseColor = new BABYLON.Color3(1.0, 0.8, 0.8);
        solar1.material = msolars;
        solar2.material = msolars;
    ...
    //initialize
    createSpeed(){    
        var GM = 0.3;  //Gravitational constant
        var v = [];
        v[0] = Math.sqrt(GM/this.satelliteM.position.z); //initial tangential velocity
        v[1] = 0;
        return v;
    }
    ...
    addSample(sample){
         ...
         this.r = sample.rho_m;
         var theta = sample.theta_m;
         var w_0 = sample.w_m;
         
         var vx_m = w_0*this.r*Math.sin(theta);
         var vz_m = w_0*this.r*Math.cos(theta);
         this.x_m = this.r*Math.sin(theta);
         this.z_m = this.r*Math.cos(theta);
         ...
         this.satelliteM.position.x = this.x_m;
         this.satelliteM.position.z = this.z_m;
         }
        
         this.theta_m = theta;
         //this.satelliteM.rotation.y = this.theta_m+Math.PI/2;
        
         var inpos_m= new BABYLON.Vector3(this.satelliteM.position.x - vx_m,0,this.satelliteM.position.z - vz_m);
         var circle_m = BABYLON.MeshBuilder.CreateLines("circle_m", {points:[inpos_m,new BABYLON.Vector3(this.satelliteM.position.x,0,this.satelliteM.position.z)]},scene);
         this.line.push(circle_m);
         if (this.line.length>600){
            this.line.shift().dispose();
         }
         circle_m.color = new BABYLON.Color3(0, 1, 0.1);
    ...
    push_animation(){
        this.ps1 = new BABYLON.ParticleSystem("particles", 100, scene);
        ...
        this.ps1.start();
        this.ps1.targetStopDuration = 20;
        //this.satelliteM.rotation.y = this.theta_m+Math.PI/2;
    }
    
    pull_animation(){
        this.ps2 = new BABYLON.ParticleSystem("particles", 100, scene);
        ...
        this.ps2.start();
        this.ps2.targetStopDuration = 20;
        //this.satelliteM.rotation.y = this.theta_m+Math.PI/2;
    }
    ...
    end_action(){
        ...
        sleep(2000).then(() => {
            this.satelliteM.rotation.y = 0;
        });
    }
    
    explosion_start(){

        this.smokeSystem = new BABYLON.ParticleSystem("fire", 1000, scene);
	    this.smokeSystem.particleTexture = new BABYLON.Texture("lib/textures/flare.png", scene);
	    this.smokeSystem.emitter = this.satelliteM; // the starting object, the emitter
        ...
        this.smokeSystem.direction1 = new BABYLON.Vector3(0.5*this.x_m, 0, 0.5*this.z_m);
        this.smokeSystem.direction2 = new BABYLON.Vector3(0.5*this.x_m, 0, 0.5*this.z_m);
        ...
        this.fireSystem = new BABYLON.ParticleSystem("fire", 2000, scene);

        //Texture of each particle
        this.fireSystem.particleTexture = new BABYLON.Texture("lib/textures/flare.png", scene);

        // Where the particles come from
        this.fireSystem.emitter = this.satelliteM; // the starting object, the emitter
        ...
        this.fireSystem.direction1 = new BABYLON.Vector3(0.5*this.x_m, 0, 0.5*this.z_m);
        this.fireSystem.direction2 = new BABYLON.Vector3(0.5*this.x_m, 0, 0.5*this.z_m);
    ...
    stop(){
        ...
        sleep(1000).then(() => {
            this.satelliteM.position.x = 0;
            this.satelliteM.position.z = 0;
        });
    }
}
